...
export async function processPerson(currentOps, personId: string, notion: any, publisher?: string): Promise<[Array<Op>, string]> {
    ...
    if (geoId = await searchOps(currentOps, SystemIds.NAME_PROPERTY, "TEXT", name, SystemIds.PERSON_TYPE)) { //Search current ops for web url
        ...
        if (await hasBeenEdited(currentOps, geoId)) {
            ...
            if ((publisher) || (await searchArticles(GEO_IDS.cryptoNewsSpaceId, geoId, GEO_IDS.authorsPropertyId))) { //Seach graph to see if they are author of any articles
                ...
                if (entityOnGeo) {
                    const blocksOnEntity = entityOnGeo?.relationsByFromVersionId?.nodes.filter(
                        (item) => 
                            ...
                            ((item.toEntity.name?.toLowerCase() === "Authored articles"?.toLowerCase()) || (item.toEntity.name?.toLowerCase() === "Articles"?.toLowerCase()))
                    );
                    if (blocksOnEntity.length < 1) {
                        addOps = createQueryDataBlock("Authored articles", geoId, filter, SystemIds.LIST_VIEW, INITIAL_RELATION_INDEX_VALUE, [GEO_IDS.relatedProjectsPropertyId]);
                        ops.push(...addOps);
                    }
                } else {
                    addOps = createQueryDataBlock("Authored articles", geoId, filter, SystemIds.LIST_VIEW, INITIAL_RELATION_INDEX_VALUE, [GEO_IDS.relatedProjectsPropertyId]);
                ...
                if ((publisher) && (!publisherList.includes(publisher))) {
