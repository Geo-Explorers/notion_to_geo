...
export async function processProject(projectId: string, notion: any): Promise<geoId> {
    ...
    if (geoId = await searchEntities(GEO_IDS.cryptoSpaceId, SystemIds.NAME_PROPERTY, name, SystemIds.PROJECT_TYPE)) { //Search graphDB for web url
        return geoId;
    } else {
        return null;
    }
